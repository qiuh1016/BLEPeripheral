...
class ViewController: UIViewController,CBPeripheralManagerDelegate {
    ...
    func terminalTextAppend(str: String){
        let originalText = terminalTextView.text
        terminalTextView.text =  "\(systemTimeHHmmss()): " + str + "\n\n" + originalText
        
    }
    
    @IBAction func clearTerminal(sender: AnyObject) {
    ...
    func peripheralManager(peripheral: CBPeripheralManager, didAddService service: CBService, error: NSError?) {
        NSLog("添加服务" )
        terminalTextAppend("didAddService")
        
        if error != nil {
            NSLog("添加服务失败: %@", error!.localizedDescription)
            terminalTextAppend("AddService failed, error: \(error!.localizedDescription)")
        }
    }
    ...
    func peripheralManager(peripheral: CBPeripheralManager, central: CBCentral, didSubscribeToCharacteristic characteristic: CBCharacteristic) {
        NSLog("中心已经预定了特征")
        terminalTextAppend("Central didSubscribeToCharacteristic")
    ...
    func peripheralManager(peripheral: CBPeripheralManager, central: CBCentral, didUnsubscribeFromCharacteristic characteristic: CBCharacteristic) {
        NSLog("中心没有从特征预定")
        terminalTextAppend("Central didUnsubscribeFromCharacteristic")
    }
    ...
    func sendData() {
        ...
        while didSend {
            ...
            if let stringFromData = NSString(data: chunk, encoding: NSUTF8StringEncoding) {
                ...
                terminalTextAppend("Sent: \(stringFromData)")
            }
